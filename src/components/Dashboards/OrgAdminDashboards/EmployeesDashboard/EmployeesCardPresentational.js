...
const EmployeesCardPresentational =(props)=>{
	const {EmployeeshandleHide,orgEmployeeTableListdata,orgEmployeeCount,page_number,handlePageChange} = props;
		console.log("props",handlePageChange);
	return (
		<div>  
			<div>
				<Card className="gx-card" title="Available Org Employees">
					<div className="cs-table">
						<table className="cs-table">
							...
							<tbody>
								{
			                  orgEmployeeTableListdata && orgEmployeeTableListdata.map((el, index) => 
			                	<tr key={index}>
				   ...
				   {orgEmployeeCount > 0  && 
		              <CommonPagination page_number={page_number} totalRecordsCount={orgEmployeeCount} 
					...
					<div className="cs-AlignRight cs-paginationCloseAlign"> 
						<Button type="primary"  className ="cs-btnHoverZoom"  onClick={EmployeeshandleHide}>Close</Button>
					</div>
				</Card>
			</div>
	    </div>
...
export default EmployeesCardPresentational;