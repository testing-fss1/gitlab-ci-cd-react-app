...
function EMailCard (props) {
	const {form: {getFieldDecorator}, styleName, headerStyle, footerStyle, GroupsListDataContainer, NotifiTypesListServiceDataList, handleFunctionData, ComposeEMailAction, ComposeEMailStatus, state :{sweet}, onConfirm, setstate} = props
	...
	const EMailSubmit = (values) => {
		...
		props.form.validateFields((err, values) => {
			if (!err) {
				console.log("EMailSubmit values : ", values)
				ComposeEMailAction(values, checkedValue, finalComparedValueNotifiTypeId)
			}
		});
	}
	...
	return (
		<div>
						...
						<div>
							...
							>
								{getFieldDecorator('EMail_message', {
									...
									<TextArea
										style={{width: "100%"}}
										placeholder="E-Mail Content Here"
					...
					{checkedValue == "Notification Type Specific User Group" && 
						<div>
							...
							<FormItem
								{...formItemLayout}
							>
								{getFieldDecorator('EMail_message_2', {
									rules: [{
										required: true,
										message: "This Field is Required"
									}]
			...
			<div style = {{textAlign: "center"}}>
				<Button type="primary" onClick = {EMailSubmit}>Submit</Button>
			</div>
		    {
				sweet && ComposeEMailStatus && ComposeEMailStatus =="notification-compose-info-insertion-success" &&
				<Sweetalert 
					visible={sweet}
...
export default Form.create()(EMailCard);
