...
function VoiceMailCard (props) {
	const {form: {getFieldDecorator}, styleName, headerStyle, footerStyle, GroupsListDataContainer, NotifiTypesListServiceDataList, handleFunctionData, ComposeVoiceMailAction, ComposeVoiceMailStatus, state :{sweet}, onConfirm, setstate} = props
	...
	const VoiceMailSubmit = (values) => {
		setstate({sweet : true});
		props.form.validateFields((err, values) => {			
			if (!err) {
				console.log("values : ", values)
				ComposeVoiceMailAction(values, checkedValue, finalComparedValueNotifiTypeId)
			}
		});
	}
	...
	return (
		<div>
					...
					{checkedValue == "Generic User Group" && 
						<div>
							...
							<FormItem
								{...formItemLayout}
							>
								{getFieldDecorator('VoiceMail_message', {
									rules: [{
										required: true,
										message: "This Field is Required"
									}]
									...
									<TextArea
										style={{width: "100%"}}
										placeholder="Voice Mail Content Here"
					...
					{checkedValue == "Notification Type Specific User Group" && 
						<div>
							...
							<FormItem
								{...formItemLayout}
							>
								{getFieldDecorator('VoiceMail_message_2', {
									rules: [{
										required: true,
										message: "This Field is Required"
									}]
			...
			<div style = {{textAlign: "center"}}>
				<Button type="primary" onClick = {VoiceMailSubmit}>Submit</Button>
			</div>
			{
				sweet && ComposeVoiceMailStatus && ComposeVoiceMailStatus =="notification-compose-info-insertion-success" &&
				<Sweetalert 
					visible={sweet}
...
export default Form.create()(VoiceMailCard);
