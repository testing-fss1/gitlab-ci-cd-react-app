...
test('Compact renders correctly', function () {
  var tree = renderer.create(React.createElement(Compact, red)).toJSON();
  expect(tree).toMatchSnapshot();
});

test('Compact with onSwatchHover renders correctly', function () {
  var tree = renderer.create(React.createElement(Compact, _extends({}, red, { onSwatchHover: function onSwatchHover() {} }))).toJSON();
  expect(tree).toMatchSnapshot();
});
...
test('Compact onChange events correctly', function () {
  ...
  var tree = mount(React.createElement(Compact, _extends({}, red, { onChange: changeSpy })));
...
test('Compact with onSwatchHover events correctly', function () {
  ...
  var tree = mount(React.createElement(Compact, _extends({}, red, { onSwatchHover: hoverSpy })));
...
test('CompactFields renders correctly', function () {
  var tree = renderer.create(React.createElement(CompactFields, red)).toJSON();
  expect(tree).toMatchSnapshot();
});

test('Compact renders custom styles correctly', function () {
  var tree = renderer.create(React.createElement(Compact, _extends({}, red, { styles: { default: { wrap: { boxShadow: '0 0 10px red' } } } }))).toJSON();
