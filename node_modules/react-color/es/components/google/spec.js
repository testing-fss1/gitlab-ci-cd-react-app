...
test('Google renders correctly', function () {
  var tree = renderer.create(React.createElement(Google, red)).toJSON();
...
test('Google onChange events correctly', function () {
  ...
  var tree = mount(React.createElement(Google, _extends({}, red, { onChange: changeSpy })));
  expect(changeSpy).toHaveBeenCalledTimes(0);
});

test('GoogleFields renders correctly', function () {
  var tree = renderer.create(React.createElement(GoogleFields, red)).toJSON();
  expect(tree).toMatchSnapshot();
});
