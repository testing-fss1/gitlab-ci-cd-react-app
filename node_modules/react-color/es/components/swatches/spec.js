...
test('Swatches renders correctly', function () {
  var tree = renderer.create(React.createElement(Swatches, { hex: red.hex, colors: [['#fff'], ['#333']] })).toJSON();
  expect(tree).toMatchSnapshot();
});

test('Swatches renders custom styles correctly', function () {
  var tree = renderer.create(React.createElement(Swatches, { hex: red.hex, colors: [['#fff'], ['#333']], styles: { default: { picker: { boxShadow: '0 0 10px red' } } } })).toJSON();
...
test('SwatchesGroup renders correctly', function () {
  var tree = renderer.create(React.createElement(SwatchesGroup, { active: red.hex, group: ['#fff'] })).toJSON();
  expect(tree).toMatchSnapshot();
});