...
test('Github renders correctly', function () {
  var tree = renderer.create(React.createElement(Github, red)).toJSON();
...
test('Github `triangle="hide"`', function () {
  var tree = renderer.create(React.createElement(Github, _extends({}, red, { triangle: 'hide' }))).toJSON();
  expect(tree).toMatchSnapshot();
});

test('Github `triangle="top-right"`', function () {
  var tree = renderer.create(React.createElement(Github, _extends({}, red, { triangle: 'top-right' }))).toJSON();
  expect(tree).toMatchSnapshot();
});

test('Github renders custom styles correctly', function () {
  var tree = renderer.create(React.createElement(Github, _extends({}, red, { styles: { default: { card: { boxShadow: '0 0 10px red' } } } }))).toJSON();
